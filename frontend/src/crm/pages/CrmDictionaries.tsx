...
export default function CrmDictionaries() {
  ...
  const create = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await crmCreateDictionary(entity, { name });
      setName('');
      await load();
    } catch (e: any) {
  ...
  const update = async (row: Record<string, any>) => {
    ...
    if (!nextName) return;
    try {
      await crmUpdateDictionary(entity, String(row.id), { name: nextName });
      await load();
    } catch (e: any) {
  ...
  const remove = async (row: Record<string, any>) => {
    ...
    try {
      await crmDeleteDictionary(entity, String(row.id));
      await load();
    } catch (e: any) {
  ...
  return (
    <div className="p-6 max-w-5xl">
      ...
      <div className="rounded-2xl border bg-background divide-y">
        {rows.map(row => (
          <div key={String(row.id)} className="p-4 flex items-center gap-2">
